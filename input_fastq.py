...
def parse_and_link(file, symlink_dir, metadata_dict):
    ...
    match_trimmed = re.match(r".*?/.*?/.*?/([a-zA-Z0-9-.]+)_([^_/]+)_L([0-9]+)_R(1|2).trimmed-paired.fastq.gz",file)
    if match_old:
        ...
        run_id = match_trimmed.group(2)
        sample = match_trimmed.group(1)
        lane = int(match_trimmed.group(3))
        pair = match_trimmed.group(4)
        encoding = 'S'
    else:
    ...
    if match_old:
        ...
        newfile = os.path.join(symlink_dir, "%s_%s_L%d_%s_trimmed.fastq.gz" % 
                                    (sample, run_id, lane, pair))
